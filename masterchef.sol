...
library Address {
     ...
     * of certain opcodes, possibly foxy contracts go over the 2300 gas limit
     * imposed by `transfer`, foxy them unable to receive funds via
...
library SafeERC20 {
    ...
    function _callOptionalReturn(IERC20 token, bytes memory data) private {
        // We need to perform a low level call here, to bypass Solidity's return data size checfoxy mechanism, since
 ...
 * `nonReentrant` may not call one another. This can be worked around by foxy
...
contract ReentrancyGuard {
     ...
     * by foxy the `nonReentrant` function external, and make it call a
...
contract FoxyToken is ERC20('FoxyToken', 'FOXY') {
    ...
    /// @notice A checkpoint for marfoxy number of votes from a given block
    struct Checkpoint {
        uint32 fromBlock;
        uint256 votes;
    }
...
// MasterChef is the master of FOXY. He can make foxy and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accFoxyPerShare;   // Accumulated Yieldes per share, times 1e18. See below.
    ...
    // The Foxy TOKEN!
    FoxyToken public immutable Foxy;
    address public devAddress;
    address public feeAddress;
    

    // Foxy tokens created per block.
    uint256 public FoxyPerBlock = 0.55 ether;
    
    uint256 constant MAX_FOXY_PERBLOCK= 25 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 FoxyPerBlock);
    ...
    // Update the given pool's Foxy allocation point and withdraw fee. Can only be called by the owner.
    ...
    // View function to see pending Foxyes on frontend.
    function pendingFoxy(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            ...
            accFoxyPerShare = accFoxyPerShare.add(FoxyReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accFoxyPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 FoxyReward = multiplier.mul(FoxyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Foxy.mint(devAddress, FoxyReward.div(10));
        Foxy.mint(address(this), FoxyReward.mul(80).div(100));
        // Automatically burn 10% of minted tokens
        Foxy.mint(BURN_ADDRESS, FoxyReward.mul(10).div(100));
        pool.accFoxyPerShare = pool.accFoxyPerShare.add(FoxyReward.mul(80).div(100).mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Foxy allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accFoxyPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accFoxyPerShare).div(1e18);
    ...
    // Safe Foxy transfer function, just in case if rounding error causes pool to not have enough Foxy.
    function safeFoxyTransfer(address _to, uint256 _amount) internal {
        uint256 FoxyBal = Foxy.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > FoxyBal) {
            transferSuccess = Foxy.transfer(_to, FoxyBal);
        } else {
            transferSuccess = Foxy.transfer(_to, _amount);
        }
        require(transferSuccess, "safeFoxyTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _FoxyPerBlock) external onlyOwner {
        require(_FoxyPerBlock<=MAX_FOXY_PERBLOCK,"Too high");
        massUpdatePools();
        FoxyPerBlock = _FoxyPerBlock;
        emit UpdateEmissionRate(msg.sender, _FoxyPerBlock);
    }
